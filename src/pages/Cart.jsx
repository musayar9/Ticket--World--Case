  ...
  return (
    <>
      ...
      <div className="p-2 max-2xl:w-[50%] max-xl:w-[60%] max-lg:w-[70%] max-md:w-[90%] max-sm:w-[100%] m-auto mt-24 flex flex-col items-center">
        {cartList?.length !== 0 ? (
          cartList?.map((concert, index) => (
            <CartCard
              key={index}
              item={concert.item}
              selectedSeats={concert.selectedSeats}
            />
          ))
        ) : (
          <>
            <div
              className="p-4 my-7 w-[100%] text-center text-lg text-blue-800 rounded-lg bg-blue-100 mt-20"
              role="alert"
            >
        ...
        <div className="w-[100%] flex text-gray-800 justify-between items-center p-2 border border-gray-700 rounded-lg shadow">
          ...
          <p className="mb-3 font-bold text-gray-700">{`${formatPrice(
            totalCost.toFixed(2)
          )}`}</p>
          ...
          <Link to="/payment">
            <button
              type="button"
              className={`text-white bg-green-700 hover:bg-green-800 font-medium rounded-lg text-sm px-3 py-2 text-center inline-flex items-center dark:bg-green-600 dark:hover:bg-green-700 ${cartList?.length !== 0 ? "opacity-100" : "opacity-50"
                }`}
