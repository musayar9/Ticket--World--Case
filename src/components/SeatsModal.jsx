  ...
  const handleSelectSeat = (rowIndex, columnIndex) => {
    ...
    setSelectedSeats((prev) => {
      if (
        prev.some(
          (seat) =>
            seat.rowIndex === rowIndex && seat.columnIndex === columnIndex
        )
      ) {
        return prev.filter(
          (seat) =>
  ...
  return (
    <>
      ...
      <div
        ...
        tabIndex="-1"
        aria-hidden="true"
        className={`flex items-center justify-center fixed top-0 left-0 right-0 z-50  ${
              ...
              {[...new Array(7)].map((row, rowIndex) => (
                <div key={rowIndex} className="row">
                  {[...new Array(21)].map((column, columnIndex) => (
                    <span
                      onClick={(e) =>
                        handleSelectSeat(rowIndex + 2, columnIndex + 1)
                      }
                    >
                      {" "}
                      <ChairIcon
                        className={`${
                          selectedSeats.some(
                            (seat) =>
                              seat.rowIndex === rowIndex + 2 &&
                              seat.columnIndex === columnIndex + 1
                          )
                            ? "text-green-800 "
                            : "text-red-800"
