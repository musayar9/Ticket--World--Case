...
export default function Signup() {
    ...
    const handleSubmit = async (values) => {
        ...
        const filtered = responseData.find((item) => item.email === values.email);

        if (filtered !== undefined) {
            console.log("Bu kullanıcı var");
        } else {
            axiosUserApi.post("/users", values);
    ...
    return (
        <div className="container mx-auto my-14 max-2xl:w-[35%] max-xl:w-[55%] max-lg:w-[65%] max-sm:w-[100%]">
            ...
            <div className="border border-solid border-gray-300 p-4 my-2" role="group">
                <label htmlFor="terms" className="ml-2 text-sm">
                    Do you already have an account? <a href="#" className="text-blue-600 hover:underline dark:text-blue-500">Log in</a>
                </label>
            </div>
        </div>
    );
